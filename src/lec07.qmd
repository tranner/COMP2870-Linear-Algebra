...
::: {#exm-projection}
Let $P$ be the 3x3 matrix that represents projection on to a plane $\Pi =
...
TODO possibly remove if we don't do scaling/shifting methods.
...
Although this procedure is gives us back eigenvalues there are cases where
...
real eigenvalues (zero imaginary part) and its eigenvectors are distinct and
orthogonal.
:::
...
::: {#cor-evector-basis}
Let $A$ be a symmetric $n \times n$ matrix with real entries. Then the
eigenvectors of $A$ form a basis of $\mathbb{R}^n$.
:::

::: {.proof}
We can only given an incomplete proof of this result. We will show that the
