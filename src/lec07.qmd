...
We are thinking of a matrix $A$.
We are interested in when the matrix-vector product $A \vec{x}$ is *parallel* to
$\vec{x}$.

::: {#def-evalues}
We say that any non-zero vector $\vec{x}$ is an *eigenvector* of $A$ if $A \vec{x}$ is parallel to
$\vec{x}$. Here by parallel, we mean that
there exists a number $\lambda$ (which can be positive, negative or zero, real
...
If there exists a nonzero vector $\vec{x}$ with $A \vec{x} = \vec{0}$, then
$\vec{x}$ is an eigenvector associated with the eigenvalue $0$. In fact, we know
that $0$ is an eigenvalue of $A$ if, and only if, $A$ is singular.
...
\end{equation*}
This means that $s \vec{y}$ is also an eigenvector of $P$ associated with the
...
## Why does a computer scientist need to know about eigenvalues

Eigenvalues and eigenvectors can be used to encode information about a dataset
into a lower dimensional representation. Take, for example, the health record
of a patient which might contain many dimensions of data (e.g. height, weight,
resting heart rate, different blood test results, diet and exercise information).
This dataset will be hard to compute with because these columns are not independent
(more on this later in the Statistics section of the module) and the large
dimension makes computation expensive. One way to reduce the dimensionality is
through projecting the dataset onto a small number of directions which come from
the eigenvectors of a related matrix (the covariance or correlation matrix). This is called
principal component analysis (PCA) and you will learn more about this in a project
later in this module.

We demonstrate briefly here how PCA can work for compressing an image.
Consider this image of the Bragg building:

Step 1: we load the image and do some normalisation:
```{python}
# | echo: True
image_url = "https://thumb.wikimedia.org/wikipedia/commons/thumb/3/31/Sir_William_Henry_Bragg_Building_-_Main_entrance_-_July_2022.jpg/960px-Sir_William_Henry_Bragg_Building_-_Main_entrance_-_July_2022.jpg"

response = requests.get(image_url, headers={"User-Agent": "Mozilla/5.0"})
img = Image.open(BytesIO(response.content)).convert("L")
img = img.resize((300, 300))  # reduce size
...
def reconstruct_image(data, eigenvectors, mean_vector, k):
    ...
    compressed_data = data @ top_eigenvectors
    reconstructed = compressed_data @ top_eigenvectors.T + mean_vector
...
plt.subplot(1, len(k_values), 1)

for i, k in enumerate(k_values):
    reconstructed_img = reconstruct_image(
        centered_data, eigenvectors, mean_vector, k
    )
...
and give us back the zero-vector. This happens when $(A - \lambda I_n)$ is
*singular* (@thm-det).
...
All the examples of finding eigenvectors we will see, you will be able to solve
...
\end{equation*}
By factorisation, we can read off that the values of $\lambda$ are $4$ and $2$.
...
So $(-1, 1)^T$ is an eigenvector associated with $2$.
:::
...
\begin{equation*}
(A + 3 I_n) \vec{x} = \lambda \vec{x} + 3 \vec{x} = (\lambda + 3) \vec{x}.
\end{equation*}
So we see that $\vec{x}$ is also an eigenvector of $A + 3 I_n$ and the associated
eigenvalue is $\lambda + 3$.
...
::: {#exm-rotation}
Let $Q$ denote the $2 \times 2$ matrix that rotates any vector anticlockwise by $\pi/2$
($=90^\circ$):
\begin{equation*}
Q = \begin{pmatrix}
0 & -1 \\ 1 & 0
...
\begin{equation*}
\det (Q - \lambda I_n) = \det \begin{pmatrix}
-\lambda & -1 \\ 1 & -\lambda
\end{pmatrix}
= \lambda^2 + 1.
\end{equation*}
...
\end{align*}
Therefore, the characteristic polynomial factors completely without any further computation.
...
larger matrices then we need a different approach!
...
and $\Lambda$ (this is the capital version of $\lambda$) to be the diagonal matrix with the eigenvalues
...
*similar matrices*. The matrix $S$, above, plays the role of $P$ in the definition here.
...
Since $P$ is invertible, we can pre-multiply this equation by $P^{-1}$ to obtain
$B = P^{-1} A P$, and post-multiply by $P^{-1}$ to obtain $A = P B P^{-1}$.
Thus, we infer that being similar is a symmetric property.

::: {#rem-eigenvalue-sim}
The matrix form of the eigenvalue equation \eqref{eq:evalue-matrix} says that the matrix $A$ is similar to the diagonal matrix of eigenvalues, **if the matrix $S$ is invertible.**
:::
...
& = P \lambda \vec{x} && \text{(since $\vec{x}$ is an eigenvector of $B$)} \\
...
The key idea behind the methods we will use to compute eigenvalues is to apply a
...
= \begin{pmatrix} 1 & 0 \\ 0 & 1 \end{pmatrix} \begin{pmatrix} 1 & 0 \\ 0 & 2 \end{pmatrix}.
$$

For $B$, we can compute the characteristic equation:
$$
0 = \det (B - \lambda I_2)
= \det \begin{pmatrix} 1 - \lambda & - 2 \\ 0 & 2 - \lambda \end{pmatrix} = (1 - \lambda)(2 - \lambda).
$$
...
= (B - 1 I_2) \begin{pmatrix} x \\ y \end{pmatrix}
= \begin{pmatrix} 0 & -2 \\ 0 & 1 \end{pmatrix} \begin{pmatrix} x \\ y \end{pmatrix}
= \begin{pmatrix} -2y \\ y \end{pmatrix}.
$$
...
$$
\begin{pmatrix} 0 \\ 0 \end{pmatrix}
= (B - 2 I_2) \begin{pmatrix} x \\ y \end{pmatrix}
= \begin{pmatrix} -1 & -2 \\ 0 & 0 \end{pmatrix} \begin{pmatrix} x \\ y \end{pmatrix}
= \begin{pmatrix} -x - 2y \\ 0 \end{pmatrix}.
$$
...
= \begin{pmatrix} 1 & -2 \\ 0 & 1 \end{pmatrix} \begin{pmatrix} 1 & 0 \\ 0 & 2 \end{pmatrix}.
$$
...
$$
Let $P = \begin{pmatrix} 1 & -2 \\ 0 & 1 \end{pmatrix}$ - this is our candidate for the similarity definition.
We need to check first that $P$ is invertible:
$$
...
In fact, this is the idea we used in @exm-similar-matrices.

## Eigenvalues of symmetric matrices {#sec-symmetric-nice}
...
real eigenvalues (zero imaginary part) and any eigenvectors with distinct
eigenvalues are orthogonal.
:::

Knowing the proof of this result is not essential for this module and is only
...
\begin{equation*}
\bar{(A \vec{x})}_i = \bar{\left(\sum_{j=1}^n A_{ij} x_j\right)}
= \sum_{j=1}^n A_{ij} \bar{x_j} = (A \bar{\vec{x}})_i.
\end{equation*}
...
eigenvalues. Then the eigenvectors of $A$ form a basis of $\mathbb{R}^n$.
:::
...
- Real symmetric matrices have real eigenvalues and (if distinct) orthogonal
