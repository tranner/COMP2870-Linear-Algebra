...
If $A \vec{x} = \vec{0}$, then $\vec{x}$ is an eigenvector associated to the
eigenvalue $0$. In fact, we know that $0$ is an eigenvalue of $A$ if, and only
if, $A$ is singular.

::: {#exm-projection}
Let $P$ be the 3x3 matrix that represents projection on to a plane $\pi$. What
...
# from mpl_toolkits.mplot3d import Axes3D # TODO is this needed?
...
- Let $\vec{x} = (1, 1)^T$, the swapping the components of $\vec{x}$ gives back
...
Eigenvalues and eigenvectors can be used to completely describe the
...
a number $\lambda$ such that $(A - \lambda I_n)$ can multiply a matrix and give
us back the zero-vector. This happens when $(A - \lambda I)$ is *singular*.

One way to test if a matrix is singular, is if the determinant is 0. This gives
us a test we can use to determine eigenvalues:
\begin{equation}
\label{eq:char}
\det( A - \lambda I_n ) = 0.
\end{equation}
...
We want to note that this example is actually surprisingly similar to
@exm-permuatation. We see that the eigenvectors are actually the same!
...
Although this procedure is robust to finding eigenvalues there are cases where
we have to be a bit careful. We have seen one example above with a
two-dimensional eigenspace associated with one eigenvector (@exm-projection).
Here are two other cases we must be careful:
...
solutions to this equation are $\pm i$. This means our algorithms for finding
eigenvalues and eigenvectors need to handle complex numbers too.
:::
...
Thus all eigenvector have the form $(x, 0)^T$ and point in the same direction as
$(1, 0)^T$.
:::
...
arbitrary square matrix. It turns out this method can be used to prove the
...
The equation \eqref{eq:SLamSinv} is an example of a more general idea of
*similar matrices*. We say that two matrices $A$ and $B$ are similar if there
...
To show any eigenvalue of $B$ is an eigenvalue of $A$, we simply repeat the
...
For the inner product of two complex vectors, we additionally take the complex
conjugate of the entries of $\vec{b}$. We call this the Hermitian product and
write $\langle \vec{a}, \vec{b} \rangle = \vec{a} \cdot \bar{\vec{b}} =
\sum_{i=1}^n a_i \bar{b_i}$. We denote the conjugate transform of a matrix $A$
by $A^H = \bar{A}^T$. We say that $A$ is *Hermitian* if $A^H = A$ and say that
if $\langle \vec{a}, \vec{b} \rangle = 0$ that $\vec{a}, \vec{b}$ are
*orthogonal*.
...
\end{aligned}
\end{equation}
In particular this means that if $A = A^H$ then $\langle A \vec{a}, \vec{b}
\rangle = \langle \vec{a}, A \vec{b} \rangle$. We call a matrix *Hermitian* if
$A = A^H$.
...
a_i|^2.
\end{align*}
Since $|a_i|$ is always a non-negative real number, we have that $\langle
...
real eigenvalues (zero imaginary part) and its eigenvectors are orthogonal.
:::
...
\end{equation*}
This implies that if $\lambda$ and $\mu$ are distinct, that $\langle \vec{x},
\vec{y} \rangle = 0$.
:::
