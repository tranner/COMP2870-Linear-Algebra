...
def plot_3d_plane_and_vectors():
    ...
    Z = (-normal_vector[0] * (X - point_on_plane[0]) -
         normal_vector[1] * (Y - point_on_plane[1])) / normal_vector[2] + point_on_plane[2]

    ax.plot_surface(X, Y, Z, alpha=0.5, color='lightblue', rstride=1, cstride=1, edgecolor='none')

    # Vector in the plane
    in_plane_vector = np.array([1, -1, 0])
    in_plane_vector = in_plane_vector - np.dot(in_plane_vector, normal_vector) * normal_vector
    ...
    projection = arbitrary_vector - np.dot(arbitrary_vector, normal_vector) * normal_vector
