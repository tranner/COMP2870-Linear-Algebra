...
opt, cov = curve_fit(lambda x, a: a*np.sqrt(x), n_values, sweep_values, 1)

plt.semilogx(n_values, sweep_values)
plt.semilogx(n_values, opt[0]*np.sqrt(n_values), label="sqrt fit")
...
```{python}
opt, cov = curve_fit(lambda x, a: a*x**3.5, n_values, times, 1)

plt.loglog(n_values, sweep_values)
plt.loglog(n_values, opt[0]*np.array(n_values)**3.5, label="sqrt fit")
