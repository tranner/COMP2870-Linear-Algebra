...
This section of the notes supports the final learning outcome around eigenvalues
and eigenvectors.
...
Addition and subtraction is relatively straight forward: we simply treat the
...
of Linear Algebra, we will need to work with complex. The definitions are
mostly what you would expect but with some slight subtleties.
...
Replacing all terms with complex value and the scalar product with Hermitian
product, we get a similar calculation:
\begin{align*}
\langle A \vec{x}, \vec{y} \rangle
& = \sum_{i=1}^n (A \vec{x})_i \bar{y}_i && \text{(definition of Hermitian product)} \\
...
and we would like something the generalises the idea of a symmetric matrix to
