...
for txt, z in zz.items():
    l = plt.plot(np.real(z), np.imag(z), "o")
    plt.text(np.real(z) + 0.1, np.imag(z) + 0.05, txt, color=l[0].get_color())
...
for key, z in zz.items():
    ...
    if im_txt != "0":
        txt += f"{im_txt}i"
    plt.text(np.real(z) + 0.1, np.imag(z) + 0.05, f"${txt}$", color=l[0].get_color())
