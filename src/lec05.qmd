...
def system_size(A, b):
    ...
    A : numpy.ndarray
        ...
        the linear system.
    b : numpy.ndarray
        A array of shape ``(n, 1)`` representing the right-hand side vector.
...
In exact arithmetic, they produce the solution after a finite number of operations,
but this fixed amount of work may be **very** large.

For a general $n \times n$ system of linear equations $A \vec{x} = \vec{b}$, the
computational cost of direct methods is typically $O(n^3)$. The amount of storage
...
function (which comes as part of the definition of the method). We will need to
choose a starting value $\vec{x}^{(0)}$ but there is often a reasonable
...
for some matrix $P$.
We call $\vec{r}^{(k)} = \vec{b} - A \vec{x}^{(k)}$ the **residual**.
...
1. Consider
   ...
   Each iteration is very expensive to compute since it requires applying $A^{-1}$ but
...
The key point here is that we want a method that is both computationally cheap
and converges quickly to the solution.
To achieve this we need to choose $P$ so that:

- $P$ is easy to compute, or the matrix-vector product $\vec{r}^{(k)} \mapsto P
   \vec{r}^{(k)}$ is easy to compute,
...
The Jacobi iteration updates all components using the values from iteration $k$.
It updates all elements of $\vec{x}^{(k)}$ *simultaneously* to
...
Note that the only difference between the formulae for Iteration 1 and 2 is the
...
In practice, we do not form the inverse of $D + L$ explicitly.
Since $D + L$ is lower triangular, systems involving $D + L$ can be solved efficiently
...
$$
\begin{aligned}
x^{(2)}_1 & = x^{(1)}_1 + \frac{1}{A_{11}} (b_1 - A_{11} x^{(1)}_1
- A_{12} x^{(1)}_2) \\
...
- On the examples @exm-jacobi-iteration and @exm-gauss-seidel-iteration, both methods converge, but relatively slowly. They might not converge at
    all!
...
Take one iteration of (a) Jacobi iteration and (b) Gauss-Seidel iteration to
...
::: {#exr-gs-harder}
1. Show that the componentwise operations written above are equivalent to $P = (D+L)^{-1}$ in \eqref{eq:general-iteration}.
2. Then show that the componentwise operations are the same as forward substitution applied to \eqref{eq:general-iteration} written as
   $$
   \vec{x}^{(k+1)} = \vec{x}^{(k)} + \vec{z}^{(k)}, \qquad
   (D + L) \vec{z}^{(k)} = (\vec{b} - A \vec{x}^{(k)}).
   $$
:::
...
thought about systems of linear equations (@exm-sparse-matrix). Sparse
matrices are very common in applications and have a structure which is very
useful when used with iterative methods. There are two main ways in which sparse
...
non-zero entries (where $\alpha$ is independent of $n$) -- in other words,
that the number of nonzero entries grows proportionally to $n$, rather than $n^2$.
...
for n in ns:
    ...
    timing_jac_sparse.append(end_time - start_time)

    start_time = time.perf_counter()
    jacobi_iteration(A_dense, b, x0, max_iter=10, verbose=False)
    end_time = time.perf_counter()

    timing_jac_dense.append(end_time - start_time)

    start_time = time.perf_counter()
    gauss_seidel_iteration_sparse(
        A_real, I_row, I_col, b, x0, max_iter=10, verbose=False
    )
    end_time = time.perf_counter()

    timing_gs_sparse.append(end_time - start_time)

    start_time = time.perf_counter()
    gauss_seidel_iteration(A_dense, b, x0, max_iter=10, verbose=False)
    end_time = time.perf_counter()

    timing_gs_dense.append(end_time - start_time)
```

```{python}
# | echo: false
plt.loglog(ns, timing_jac_sparse, "C1o", label="Jacobi sparse")
plt.loglog(ns, timing_jac_dense, "C0o", label="Jacobi dense")

plt.loglog(ns, timing_gs_sparse, "C3o", label="GS sparse")
plt.loglog(ns, timing_gs_dense, "C2o", label="GS dense")

poly_sparse = np.poly1d(
    [
        np.polyfit(
            ns,
            [
                0.5 * (a + b)
                for a, b in zip(
                    timing_jac_sparse, timing_gs_sparse, strict=False
                )
            ],
            1,
        )[0],
...
poly_dense = np.poly1d(
    [
        np.polyfit(
            ns,
            [
                0.5 * (a + b)
                for a, b in zip(timing_jac_dense, timing_gs_dense, strict=False)
            ],
            2,
        )[0],
        0,
...
plt.loglog(ns, poly_sparse, "C4--", label="$O(n)$")
plt.loglog(ns, poly_dense, "C5--", label="$O(n^2)$")
...
the *Euclidean norm* of an array (which you met last year!). The Euclidean norm,
or norm for short, is defined to be the square root of the sum of squares of the
...
methods) to use, we can use the following stopping criteria:
...
In the second and third cases, we call $tol$ the **convergence tolerance** and
the choice of $tol$ controls the accuracy of the solution.

::: {#rem-error-vs-residual}
We note that the residual $\vec{r}^{(k)}$ is different to the error $\vec{x} - \vec{x}^{(k)}$.
The error cannot be computed in general since it requires knowledge of the exact solution $\vec{x}$.
The residual can be computed and is therefore often used to monitor convergence.
...
required! Their usefulness for general dense matrix systems is very limited.
However, for sparse systems, they can offer dramatic reductions in both storage
