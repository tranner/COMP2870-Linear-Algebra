...
We use a value in brackets in the superscript to denote the iteration number to avoid confusion between the iteration number and the component of the vector:
$$
...
We call $\vec{b} - A \vec{x}^{(k)} = \vec{r}$ the **residual**. Note the above
bad examples could be written in the form of \eqref{eq:general-iteration} with
$P = O$ (the zero matrix) or $P = A^{-1}$.

## Jacobi iteration

One simple choice for $P$ in \eqref{eq:general-iteration} is given by the Jacobi
method where we take $P = D^{-1}$ where $D$ is the diagonal of $A$:
$$
...
run times of using the two different implementations of the Jacobi method both
