...
def system_size(A, b):
    ...
    inputs: numpy arrays A, b with shapes (n, m) and (nb,)
    outputs: size of system n (== m == nb)
    """
    n, m = A.shape
    nb = b.shape[0]
...
def print_array(array, array_name=None, end=None):
    """
    print the input array nicely

    inputs: numpy array to be printed
            array_name (optional) name to print at start of output,
                                  will be guessed otherwise
            end of line character (can be None)
    """
    # find array_name
