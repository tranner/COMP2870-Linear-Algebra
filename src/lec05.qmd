...
and converges quickly to the solution. One way to do this is to construct an
iteration given by
...
We call $\vec{b} - A \vec{x}^{(k)} = \vec{r}$ the **residual**. Note that the
above bad examples could be written in the form of \eqref{eq:general-iteration}
with $P = O$ (the zero matrix) or $P = A^{-1}$.
