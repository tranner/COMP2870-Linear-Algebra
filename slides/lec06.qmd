...
> Today, building theory for our algorithm for solving eigenvalue problems

Online notes: 6.6, 7.3, 7.4
...
\begin{align*}
\| \vec{z} \| = \sqrt{ \sum_{i=1}^n |z_i|^2 } = \sqrt{ \langle \vec{z}, \vec{z}
\rangle }.
\end{align*}
...
A S R^{-1} = S R^{-1} B.
\end{align*}
If $S$ is also invertible, then $P = S R^{-1}$ is also invertible and $A$ and $B$
are similar.
